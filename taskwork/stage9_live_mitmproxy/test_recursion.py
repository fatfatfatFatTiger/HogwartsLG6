...
class Counter:
    def __init__(self):
        self.num = 0
        self.first_stock_symbol = "SZ000651"
        self.second_stock_symbol = "SH600036"
        self.third_stock_symbol = "SZ002475"

    def response(self, flow: http.HTTPFlow):
        # 判断是否是想要的请求信息，通过url进行判断
        # 此处相同的请求有两个，新增判断条件，只筛选股票的接口，不筛选包含大盘的接口
        if ("https://stock.xueqiu.com/v5/stock/batch/quote.json?_t" in flow.request.pretty_url) \
                and ((self.first_stock_symbol in flow.request.pretty_url)
                     or (self.second_stock_symbol in flow.request.pretty_url)
                     or (self.third_stock_symbol in flow.request.pretty_url)):
            # 获取响应结果
    ...
    def json_travel(self, response_data):
        ...
        if isinstance(response_data, dict):  # {"a":{"b":{"c":1}}}
            ...
            for key, value in response_data.items():
                if key == "quote":
                    if response_data[key]["symbol"] == self.second_stock_symbol:
                        response_data[key]["name"] = response_data[key]["name"] * 2
                    elif response_data[key]["symbol"] == self.third_stock_symbol:
